...
void assertFile(Paperstack &object){
    object.output("C:\\Study\\Qt\\test.txt");
    Paperstack *inputPs = new Paperstack();
    inputPs->input("C:\\Study\\Qt\\test.txt");
    Iterator iterf = object.begin();
    Iterator iters = inputPs->begin();
    for(iterf, iters; iterf.get() != object.end().get() && iters.get() != inputPs->end().get(); iters.incr(), iterf.incr()){
        Test::assertEquality(iterf.get(), iters.get());
    }
}

void assertStability(Paperstack &object){
    Paper randomObject(Random::realNumber(MIN_PAPER_SIZE - 100, MAX_PAPER_SIZE + 100),
                      Random::realNumber(MIN_PAPER_SIZE - 100, MAX_PAPER_SIZE + 100),
                      Random::realNumber(MIN_T - 0.03, MAX_T + 0.03),
                      Random::realNumber(MIN_XY - 100, MAX_XY + 100),
                      Random::realNumber(MIN_XY - 100, MAX_XY + 100));
    Test::assertValidity(randomObject);
    Paperstack *copiedObject = new Paperstack(object);
    copiedObject->push(&randomObject);
    Iterator it = copiedObject->begin();
    Test::assertEquality(it.get(), randomObject);
    it.incr();
    Iterator it1 = object.begin();
    for(it, it1; it.get() != copiedObject->end().get() && it1.get() != object.end().get(); it.incr(), it1.incr()){
        Test::assertEquality(it.get(), it1.get());
    }
    assert(object.count() + 1 == copiedObject->count());

    copiedObject = new Paperstack(object);
    copiedObject->pop();
    it1 = object.begin();
    it = copiedObject->begin();
    it1.incr();
    for(it, it1; it.get() != copiedObject->end().get() && it1.get() != object.end().get(); it.incr(), it1.incr()){
        Test::assertEquality(it.get(), it1.get());
    }
    ...
    copiedObject->delAll();
    assert(copiedObject->count() == 0);
}
void assertStanding(Paperstack &object){
    double M = 0;
    ...
    if (object.count() > 0){
        ...
        for (iter; iter.get() != object.end().get(); iter.incr()) {
            double m = (iter.get()->getWidth() * iter.get()->getHeight() / 1000000) * 80;
            M += m;
            sumX += m * iter.get()->getCoordinateX();
            sumY += m * iter.get()->getCoordinateY();

        }
        firstW = iter.get()->getWidth() / 2;
        firstH = iter.get()->getHeight() / 2;
        firstX = iter.get()->getCoordinateX();
        firstY = iter.get()->getCoordinateY();
        double m = (iter.get()->getWidth() * iter.get()->getHeight() / 1000000) * 80;
        M += m;
        sumX += m * iter.get()->getCoordinateX();
        sumY += m * iter.get()->getCoordinateY();
    ...
    assert(!(std::abs(xc - firstX) > firstW || std::abs(yc - firstY) > firstH) == object.isStand());
}
...
void outputToConsole(Paperstack &object){
    ...
    for(it; it.get() != object.end().get(); it.incr()){
        std::cout << "Number: " << c << std::endl <<
                     "Width: " << it.get()->getWidth() << std::endl <<
                     "Height: " << it.get()->getHeight() << std::endl <<
                     "Thickness: " << it.get()->getThickness() << std::endl <<
                     "X: " << it.get()->getCoordinateX() << std::endl <<
                     "Y: " << it.get()->getCoordinateY() << std::endl;
        c++;
    }
    std::cout << "Number: " << c << std::endl <<
                 "Width: " << it.get()->getWidth() << std::endl <<
